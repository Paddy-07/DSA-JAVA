...
document.querySelectorAll('.priority-option').forEach(option => {
    option.addEventListener('click', function() {
        ...
        // Display priority-specific explanation
        const priorityValue = this.getAttribute('data-value');
        let priorityDesc = '';
        
        if (priorityValue === 'cost') {
            priorityDesc = 'Save up to 20% on your journey with our cost-optimized routes';
        } else if (priorityValue === 'comfort') {
            priorityDesc = 'Enjoy premium comfort with a minimum 3.5-star rating';
        } else if (priorityValue === 'time') {
            priorityDesc = 'Get there faster with our time-optimized routes';
        }
        
        // Show the description if there's already a description element
        const existingDesc = document.querySelector('.priority-description');
        if (existingDesc) {
            existingDesc.textContent = priorityDesc;
        } else {
            // Create a new description element
            const descEl = document.createElement('div');
            descEl.className = 'priority-description';
            descEl.textContent = priorityDesc;
            descEl.style.marginTop = '10px';
            descEl.style.fontSize = '0.9rem';
            descEl.style.color = '#666';
            
            // Add after the priority options
            const priorityOptions = document.querySelector('.priority-options');
            priorityOptions.parentNode.insertBefore(descEl, priorityOptions.nextSibling);
        }
    });
});
...
document.getElementById('booking-form').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        // Apply priority-specific adjustments for frontend display
        if (priority === 'cost') {
            // Apply visual discount indicator
            bookingData.cost_savings = 'Save 20%';
        } else if (priority === 'comfort') {
            // Ensure comfort rating is at least 3.5
            if (bookingData.comfort_rating < 3.5) {
                bookingData.comfort_rating = 3.5;
            }
        } else if (priority === 'time') {
            // Apply visual time savings indicator 
            bookingData.time_savings = 'Save 10% travel time';
        }
...
function displayBookingResults(data) {
    ...
    const priorityText = data.journey.priority.charAt(0).toUpperCase() + data.journey.priority.slice(1);
    document.getElementById('selected-priority').textContent = priorityText;
    
    // Add priority-specific badge if applicable
    if (data.cost_savings || data.time_savings) {
        const savingsText = data.cost_savings || data.time_savings;
        const savingsBadge = document.createElement('span');
        savingsBadge.className = 'savings-badge';
        savingsBadge.textContent = savingsText;
        savingsBadge.style.backgroundColor = data.cost_savings ? '#2ecc71' : '#3498db';
        savingsBadge.style.color = 'white';
        savingsBadge.style.padding = '3px 8px';
        savingsBadge.style.borderRadius = '12px';
        savingsBadge.style.fontSize = '0.8rem';
        savingsBadge.style.marginLeft = '10px';
        
        document.getElementById('selected-priority').appendChild(savingsBadge);
    }
    ...
    data.segments.forEach((segment, index) => {
        ...
        segmentDiv.innerHTML = `
            ...
            <div class="segment-info">
                ...
                <div class="segment-info-item">
                    <strong>Cost:</strong> ${formatCurrency(segment.cost)}
                </div>
                <div class="segment-info-item">
