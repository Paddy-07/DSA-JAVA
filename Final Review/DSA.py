...
class TransportSystem:
    ...
    def __init__(self):
        self.head = None
        self.comfort_levels = {
            'Economy': {'price_factor': 1.0, 'satisfaction': 'Basic comfort'},
            'Standard': {'price_factor': 1.3, 'satisfaction': 'Comfortable journey'},
            'Premium': {'price_factor': 1.8, 'satisfaction': 'Luxury experience'},
            'Express': {'price_factor': 1.5, 'satisfaction': 'Fast service'}
        }
    ...
    def calculate_best_route(self, start, end, priority="time"):
        ...
        traffic_conditions = {
            'low': 0.8 if not weekend else 1.0,
            'moderate': 1.0 if not weekend else 1.2,
        ...
        while not priority_queue.is_empty():
            ...
            for dest_name, comfort, cost, base_duration in current_city.connections:
                ...
                if priority == "cost":
                    ...
                    comfort_score = 4 - list(self.comfort_levels.keys()).index(comfort)
                    new_score = score + comfort_score
